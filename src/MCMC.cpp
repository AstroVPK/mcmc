...
mcmc::EnsembleSampler::EnsembleSampler(int ndims, int nwalkers, int nsteps, int nthreads, double a, double (*func)(double *x, void *funcArgs, double &LnPriorVal, double &LnLikelihoodVal), void *funcArgs, unsigned int zSeed, unsigned int bernoulliSeed, unsigned int walkerSeed) {
...
mcmc::EnsembleSampler::~EnsembleSampler() {
...
void mcmc::EnsembleSampler::runMCMC(double* initPos) {
        ...
        for (int subSetNum = 0; subSetNum < 2; subSetNum++) {
            ...
            */
            ...
            for (int walkerNum = 0; walkerNum < halfNumWalkers; walkerNum++) {
                ...
                */
                #ifdef DEBUG_RUNMCMC
                            printf("runMCMC - threadNum: %d; stepNum: %d; currWalkerNum: %d; pAccept: %e\n ",threadNum,stepNum,walkerNum+halfNumWalkers*subSetNum, pAccept);
                #endif
...
void mcmc::EnsembleSampler::getChain(double *ChainPtr) {
    int nsteps = numSteps;
    int nwalkers = numWalkers;
    int ndims = numDims;
    int sizeChain = numDims*numWalkers*numSteps;
...
void mcmc::EnsembleSampler::getChainVals(double *LnPriorPtr, double *LnLikePtr) {
    int nsteps = numSteps;
    int nwalkers = numWalkers;
    int ndims = numDims;
